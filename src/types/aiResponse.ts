...
export const FeedbackPointSchema = z
  .object({
    ...
    line_numbers: z
      .string()
      .describe(
        "The line numbers in the code where the feedback applies. When referring to bad naming, only return line numbers where that naming declared. If issue lies in several location within one function, return one line range that covers both issues. Denoted as a comma separated list, with individual numbers or ranges of numbers (e.g. 3,4,10-15)",
      ),
    severity: z
      .number()
      .int()
